...
console.log('📤 Exporting data from SQLite (final)...');

try {
  const db = new Database('server/trq.db');
  
  // Define which columns to export for each table (without updatedAt)
  const tableColumns = {
    projects: ['id', 'title', 'title_ar', 'category', 'category_ar', 'subcategory', 'subcategory_ar', 
               ...
               'clientName_ar', 'status', 'sortOrder', 'createdAt'],
    ...
    blog_articles: ['id', 'title', 'slug', 'excerpt', 'content', 'image', 'author', 'date', 
                   'readTime', 'category', 'categorySlug', 'tags', 'status', 'createdAt']
  };
  ...
  // Write to file
  fs.writeFileSync('backup-final.sql', sql, 'utf-8');
  
  const fileSize = fs.statSync('backup-final.sql').size / 1024;
  console.log(`✅ Exported to backup-final.sql (${fileSize.toFixed(2)} KB)`);
