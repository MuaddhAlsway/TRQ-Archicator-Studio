...
async function check() {
  console.log('🔍 CHECKING GALLERY PATHS\n');
  
  const projects = await executeQuery('SELECT id, title, gallery FROM projects WHERE gallery IS NOT NULL LIMIT 5');
    ...
    try {
      ...
      if (Array.isArray(gallery)) {
        gallery.forEach((img, idx) => {
          const isAbsolute = img.startsWith('https://') || img.startsWith('http://');
          const status = isAbsolute ? '✅' : '⚠️ ';
          console.log(`  ${status} [${idx}] ${img.substring(0, 80)}`);
        });
      }
    } catch (e) {
      console.log(`  ❌ Invalid JSON: ${p.gallery.substring(0, 80)}`);
    }
    console.log('');
  });
}
