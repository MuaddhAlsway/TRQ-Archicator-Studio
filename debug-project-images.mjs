...
async function debug() {
  console.log('🔍 DEBUGGING PROJECT IMAGES\n');
  
  const projects = await executeQuery('SELECT id, title, image, status FROM projects ORDER BY id ASC');
  ...
  projects.forEach(p => {
    ...
    if (!p.image) {
      ...
      console.log(`  ❌ NOT ABSOLUTE URL`);
      issues++;
    } else {
      // Check if file exists
      const filename = p.image.split('/').pop();
      console.log(`  ✅ Absolute URL - File: ${filename}`);
    }
    console.log('');
  });
