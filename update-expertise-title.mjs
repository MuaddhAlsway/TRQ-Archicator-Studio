...
async function updateExpertiseTitle() {
  try {
    ...
    const result = await executeQuery(
      `UPDATE settings SET aboutExpertise2Title = ? WHERE 1=1`,
    ...
    const verify = await executeQuery(
      `SELECT aboutExpertise2Title FROM settings LIMIT 1`,
      []
    );
    
    console.log('Verified value:', verify[0]?.aboutExpertise2Title);
  } catch (error) {
