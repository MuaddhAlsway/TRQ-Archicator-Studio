...
async function checkSchema() {
  console.log('Checking hero_slides schema...\n');

  try {
    // Get all slides
    const slides = await executeQuery('SELECT * FROM hero_slides LIMIT 1');
    
    if (slides.length > 0) {
      console.log('✓ hero_slides table exists');
      console.log('\nColumns in first slide:');
      Object.keys(slides[0]).forEach(key => {
        console.log(`  - ${key}: ${slides[0][key]}`);
      });
    }

    // Get all slides to check video field
    const allSlides = await executeQuery('SELECT id, title, video FROM hero_slides');
    console.log('\n\nAll slides with video field:');
    allSlides.forEach(slide => {
      console.log(`  ID ${slide.id}: ${slide.title}`);
