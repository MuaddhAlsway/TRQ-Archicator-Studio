...
async function checkSlides() {
  const response = await fetch(TURSO_API_URL, {
    ...
    body: JSON.stringify({
      requests: [{
        type: 'execute',
        stmt: {
          sql: 'SELECT id, title, video FROM hero_slides ORDER BY id',
          args: [],
        },
      }],
    }),
  ...
  if (data.results?.[0]?.response?.result?.rows) {
    ...
    console.log('Current slides in database:');
    rows.forEach(row => {
      const obj = {};
      cols.forEach((col, idx) => {
        obj[col.name] = row[idx]?.value;
      });
      console.log(`ID: ${obj.id}, Title: ${obj.title}, Video: ${obj.video ? 'YES' : 'NO'}`);
    });
  }
}
