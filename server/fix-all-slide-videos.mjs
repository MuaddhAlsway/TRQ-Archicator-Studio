...
try {
  console.log('Fixing All Hero Slide Videos...\n');

  // Slide 1: POV 1.mp4 only
  db.prepare(`UPDATE hero_slides SET video = ?, video_2 = NULL, video_3 = NULL WHERE id = 1`).run('/POV 1.mp4');
  console.log('✓ Slide 1: Set to POV 1.mp4 only');

  // Slide 2: Video.mp4 only
  db.prepare(`UPDATE hero_slides SET video = ?, video_2 = NULL, video_3 = NULL WHERE id = 2`).run('/Video.mp4');
  console.log('✓ Slide 2: Set to Video.mp4 only');
