...
async function fixAllPaths() {
  ...
  const projects = await executeQuery('SELECT id, image FROM projects');
  ...
  for (const project of projects) {
    ...
    if (oldPath !== newPath) {
      await executeQuery('UPDATE projects SET image = ? WHERE id = ?', [newPath, project.id]);
      console.log(`✓ Project ${project.id}: ${oldPath} → ${newPath}`);
      fixed++;
    }
  }
  
  console.log(`\n✅ Fixed ${fixed} project paths!`);
}

fixAllPaths().catch(console.error);
