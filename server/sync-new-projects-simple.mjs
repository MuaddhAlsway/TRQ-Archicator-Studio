...
try {
  ...
  const projects = localDb.prepare(`
    SELECT id, title, title_ar, description, description_ar, category, category_ar,
           image, gallery, duration, duration_ar, client, client_ar, status
    FROM projects 
    WHERE id >= 23
    ORDER BY id
  `).all();
      ...
      await turso.execute({
        sql: `
          INSERT INTO projects (
            id, title, title_ar, description, description_ar, category, category_ar,
            image, gallery, duration, duration_ar, client, client_ar, status
          ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `,
        args: [
