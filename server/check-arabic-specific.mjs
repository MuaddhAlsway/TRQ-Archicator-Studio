...
async function checkArabic() {
  console.log('🔍 Checking Arabic fields specifically...\n');
  
  const slides = await executeQuery('SELECT id, tag_ar, title_ar, description_ar FROM hero_slides WHERE id = 1');
  console.log('Slide 1 Arabic fields:', JSON.stringify(slides, null, 2));
}

checkArabic().catch(console.error);
