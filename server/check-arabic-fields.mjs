...
async function checkSlides() {
  console.log('🔍 Checking hero slides for Arabic fields...\n');
  
  const slides = await executeQuery('SELECT id, tag, tag_ar, title, title_ar FROM hero_slides ORDER BY id ASC');
  
  slides.forEach(slide => {
    console.log(`Slide ${slide.id}:`);
    console.log(`  tag: ${slide.tag}`);
    console.log(`  tag_ar: ${slide.tag_ar}`);
    console.log(`  title: ${slide.title}`);
    console.log(`  title_ar: ${slide.title_ar}`);
    console.log('');
  });
}
