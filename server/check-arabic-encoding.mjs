...
async function checkEncoding() {
  console.log('🔍 Checking Arabic encoding in database...\n');
  
  const slides = await executeQuery('SELECT id, tag_ar FROM hero_slides WHERE id = 1');
  if (slides.length > 0) {
    const text = slides[0].tag_ar;
    console.log('Raw text:', text);
    console.log('Text length:', text.length);
    console.log('Char codes:', Array.from(text).map(c => c.charCodeAt(0).toString(16)).join(' '));
  }
}
